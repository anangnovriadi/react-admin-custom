...
class Wrapper extends Component {
    ...
    componentWillMount() {
        ...
        axios.get('http://18.219.201.200:8080/api/getreceipt/'+params)
        .then((res) => {
            this.setState({
                list: res.data.data
            })
    ...
    handleClick(e) {
        e.preventDefault()
        let params = this.props.match.params.id;
        axios.get('http://18.219.201.200:8080/api/update-receipt/'+params)
        .then((res) => {
            swal({
                title: "Success",
            ...
            setTimeout(() => history.push('/transaction'), 1000);
        })
        .catch((err) => {
            console.log(err)
        })
                    ...
                    <div className="card pd-20 pd-sm-40">
                        ...
                        <div className="table-wrapper">
                            <form>
                                ...
                                <button className="btn btn-secondary" onClick={this.handleClick} type="submit">Update</button>
                            </form>
                        </div>
                    </div>
                </div>
